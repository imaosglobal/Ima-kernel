...
def answer(question, events):
    ...
    if intent == "information_request":
        ...
        return {
            "text": "אני IMA. אני אעזור לך למצוא ולהבין את המידע שאתה מחפש. שכבת המידע שלי מוכנה להתחבר לכלים נוספים.",
            "confidence": 0.7
        }
    ...
    # IMA conversation fallback

    if any(x in question for x in ["שלום", "היי", "בוקר טוב", "ערב טוב"]):
        return {
            "text": "שלום. אני כאן איתך. איך עובר עליך היום?",
            "confidence": 0.85
        }

    if any(x in question for x in ["מה נשמע", "מה קורה", "איך את", "איך הולך"]):
        return {
