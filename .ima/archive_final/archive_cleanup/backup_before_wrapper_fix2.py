...
def answer(question, events):
    ...
    if intent == "technical_request":
        ...
        if model_result:
            return {
                "text": "אני IMA. אני אסביר לך בצורה ברורה ופשוטה. " + model_result.get("text", ""),
                "confidence": model_result.get("confidence", 0.8)
            }

        return {
            "text": "אני IMA. אני רוצה לעזור לך להבין את זה. כרגע אין לי מנוע הסבר טכני מחובר, אבל אני יכולה להמשיך ללמוד ולהתחבר לכלים המתאימים.",
            "confidence": 0.7
        }
