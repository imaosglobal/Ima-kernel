...
class IMAMaster:
    ...
    def ask(self,message):
        ...
        try:
            if hasattr(ima_system, "answer"):
                sr = ima_system.answer(
                    message,
                    ima_brain.load_events() if hasattr(ima_brain,"load_events") else []
                )
                if sr:
                    system_response = sr.get("text")
        except Exception:
            system_response = None
        ...
        if "מי אני" in message:
            ...
            try:
                ...
                if any(x in message for x in memory_queries):
                    memory_hits = conversation_layer.recall(message)

                    if memory_hits:
                        result["response"] = str(memory_hits[-5:])
                    else:
