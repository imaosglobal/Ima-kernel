...
class IMAMaster:
    ...
    def ask(self,message):
        ...
        # MEMORY_READ_FASTPATH_V1
        # Direct memory lookup before expensive LLM/brain execution.
        memory_query = message.strip()
        memory_tokens = [
            x for x in re.findall(r"[A-Za-z0-9_-]{4,}|[א-ת]{3,}", memory_query)
            if x not in {"זכור", "זוכרת", "זוכר", "אתה", "את", "מה", "קוד", "הבדיקה"}
        ]

        if memory_tokens and any(
            x in memory_query
            for x in ["מה אתה זוכר", "מה את זוכרת", "תזכיר לי", "מה דיברנו", "היסטוריה"]
        ):
            fast_hits = []

            try:
                memory_file = Path(".ima/memory.json")
                if memory_file.exists():
                    raw = memory_file.read_text(encoding="utf-8")
                    if all(token.lower() in raw.lower() for token in memory_tokens):
                        fast_hits.append(
                            "מצאתי בזיכרון: " + " ".join(memory_tokens)
                        )
            except Exception:
                pass

            try:
                log_file = Path(".ima/memory_log.jsonl")
                if log_file.exists():
                    for line in log_file.read_text(encoding="utf-8").splitlines():
                        if all(token.lower() in line.lower() for token in memory_tokens):
                            fast_hits.append(line)
                            if len(fast_hits) >= 5:
                                break
            except Exception:
                pass
        ...
        # FAST MEMORY PATH: explicit memory requests bypass expensive LLM chain
        explicit_memory = any(x in message for x in [
            "זכור", "תזכור", "שמור בזיכרון", "שמרי בזיכרון",
            "remember", "save this"
        ])

        if explicit_memory:
            response = "זכרתי ושמרתי את זה בזיכרון."
            try:
                conversation_layer.update(message, response)
                saved = store_interaction(message, response)
                return {
                    "time": time.time(),
                    "message": message,
                    "response": response,
                ...
                return {
                    "time": time.time(),
                    "message": message,
                    "response": "ניסיון שמירה בזיכרון נכשל: " + str(e),
                    "memory_saved": False
                }
