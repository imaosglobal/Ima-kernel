...
def load_active_runtime_allowlist():
    from pathlib import Path
    import json

    path = Path(
        "learning/self_improvement_reports/"
        "active_runtime_allowlist.json"
    )
...
def filter_gaps_to_active_runtime(gaps):
    allowlist = load_active_runtime_allowlist()

    return [
        gap
        for gap in gaps
        if gap.get("file") in allowlist
    ]
