...
class IMAMaster:
    ...
    def ask(self,message):
        ...
        # MEMORY_READ_FASTPATH_V2
        # Canonical direct memory lookup before all expensive runtime calls.

        memory_query = message.strip()

        memory_tokens = [
            x for x in re.findall(
                r"[A-Za-z0-9_-]{4,}|[א-ת]{3,}",
                memory_query
            )
            if x not in {
                "זכור", "זוכרת", "זוכר",
        ...
        if memory_tokens and memory_trigger:
            fast_hits = []

            memory_file = __import__("pathlib").Path(".ima/memory.json")
            ...
            log_file = __import__("pathlib").Path(".ima/memory_log.jsonl")
            if log_file.exists():
