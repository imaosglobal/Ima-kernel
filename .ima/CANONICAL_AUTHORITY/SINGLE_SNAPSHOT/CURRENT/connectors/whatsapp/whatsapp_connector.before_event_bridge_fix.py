...
def emit(event_type, **data):
    try:
        ...
        # runtime is:
        # .ima/runtime
        current_root = Path(__file__).resolve().parents[5]
        stream_file = current_root / "runtime" / "stream.py"

        if not stream_file.exists():
            raise FileNotFoundError(
                f"canonical runtime stream not found: {stream_file}"
            )
...
class WhatsAppConnector:
    def emit(self, event_type, **data):
        import sys
        if ".ima/runtime" not in sys.path:
            sys.path.insert(0, ".ima/runtime")
        import stream
        return stream.emit(event_type, **data)
