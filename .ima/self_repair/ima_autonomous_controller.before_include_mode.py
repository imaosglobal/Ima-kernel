...
def snapshot():
    ...
    print("[1/3] SINGLE SNAPSHOT START", flush=True)

    stamp="CURRENT"
    target=SNAP/stamp

    # תמיד snapshot אחד בלבד
    if target.exists():
    ...
    IGNORE=[
        ".ima/self_repair/snapshots",
        ".ima/CANONICAL_AUTHORITY/HISTORY_ARCHIVE",
        ".ima/runtime_snapshots",
        ".ima/canonical_snapshots",
        ".ima/REPAIR_BACKUPS",
        ".git",
    ...
    for p in ROOT.rglob("*.py"):
        ...
        if any(x in rel_str for x in IGNORE):
            continue
    ...
    print("[1/3] SINGLE SNAPSHOT DONE:",len(files),flush=True)
