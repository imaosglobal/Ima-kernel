...
echo "=== IMA PRODUCT HEALTH LAYER ==="

mkdir -p product/health
mkdir -p .ima/reports

cat > product/health/health_manager.py <<'PY'
import json
import time
...
cat > product/health/run_health_check.py <<'PY'
...
cat > product/health/deployment_registry.json <<'JSON'
{
  "system": "IMA",
  "deployment": {
    "gateway": true,
...
echo "[CHECK] Running health"

python3 product/health/run_health_check.py

echo "=== PRODUCT HEALTH LAYER READY ==="
